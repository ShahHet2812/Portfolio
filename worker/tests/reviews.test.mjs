...
test('testimonial moderation, privacy, validation and abuse controls', async () => {
  ...
  try {
    ...
    sql.exec(readFileSync('db/migrations/0002-admin.sql','utf8'));
    const adapt = query => {
      let params=[];
    ...
    assert.equal((await app.request(`/api/reviews/photo/${row.id}?token=${token}`,{},env)).status,404);
    ...
    assert.equal(sql.prepare('SELECT status FROM testimonial_submissions').get().status,'pending');
    sql.prepare("UPDATE testimonial_submissions SET status='approved' WHERE id=?").run(row.id);
    ...
    assert.equal((await app.request(`/api/reviews/review/${token}`,{},env)).status,410);
